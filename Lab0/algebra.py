...
def do_multiply(expr1, expr2):
    ...
    * expr1 is a Product, and expr2 is a Product
    ...
    if isinstance(expr1, Sum) and isinstance(expr2, Sum):
        ...
        for fact1 in expr1:
            for fact2 in expr2:
                if isinstance(fact1,(int,str)) and isinstance(fact2,(int,str)):
                    sumlist.append(Product([fact1,fact2]))
                else:
                    sumlist.append(multiply(fact1,fact2))      
        return Sum(sumlist)
    
    if isinstance(expr1, Sum) and isinstance(expr2, Product):
        sumlist = []
        for fact1 in expr1:
            sumlist.append(multiply(fact1,expr2))
        return Sum(sumlist)
        
    if isinstance(expr1, Product) and isinstance(expr2, Sum):
        sumlist = []
        for fact2 in expr2:
            sumlist.append(multiply(expr1,fact2))
        return Sum(sumlist)

    if isinstance(expr1, Product) and isinstance(expr2, Product):
        singleProduct = Product()
        non_singleProduct = Product()
        expr3 = Product(expr2 + expr1)
        
        for fact3 in expr3:
            if isinstance(fact3,(int,str)):
                singleProduct.append(fact3)
            else:
                non_singleProduct.append(fact3)
        
        for nonfacts in non_singleProduct:
            singleProduct = multiply(singleProduct,nonfacts)
